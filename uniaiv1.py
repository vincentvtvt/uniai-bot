...
# Also extract record ID and normalize Business link
...
def fetch_config_by_service(service_number):
    ...
    # Business is a linked-record array; take first element
    biz = fields.get('Business')
    if isinstance(biz, list) and biz:
        fields['BusinessID'] = biz[0]
    else:
        fields['BusinessID'] = biz
    return fields

# Fetch config by customer number (fallback)
...
def record_history(business, config_id, phone, step, history):
    requests.post(
        ...
        json={'fields': {'Business': business, 'WhatsAppConfig': config_id, 'PhoneNumber': phone, 'CurrentStep': step, 'History': history}}
    )

# Record bookings/sales
def record_sales(business, config_id, phone, name, service):
    requests.post(
        f"{AIRTABLE_URL}/{TABLES['sales']}",
        headers={**HEADERS, 'Content-Type': 'application/json'},
        json={'fields': {'Business': business, 'WhatsAppConfig': config_id, 'PhoneNumber': phone, 'CustomerName': name, 'ServiceBooked': service, 'Status': 'Pending'}}
    )
...
def webhook():
    ...
    wa = customer_number
    business_id = cfg.get('BusinessID')
    config_id = cfg.get('RecordID')
    role = cfg.get('Role')
    ...
    # 1) Template
    tmpl = find_template(business_id, config_id, msg, lang)
    if tmpl:
        if tmpl.get('ImageURL'):
            send_image(wa, tmpl.get('ImageURL'), api_key)
        send_whatsapp(wa, tmpl.get('TemplateBody'), api_key)
        record_history(business_id, config_id, wa, tmpl.get('Step', 'step'), f'User:{msg}|Bot:{tmpl.get("TemplateBody")}')
        return jsonify({'status': 'template_sent'})

    # 2) Knowledge
    script, img_url = find_knowledge(business_id, msg, role)
    if script:
        if img_url:
            send_image(wa, img_url, api_key)
        send_whatsapp(wa, script, api_key)
        record_history(business_id, config_id, wa, 'knowledge', f'User:{msg}|Bot:{script}')
    ...
    record_history(business_id, config_id, wa, 'fallback', f'User:{msg}|Bot:{reply}')
    if any(keyword in reply.lower() for keyword in ('booking', '预约')):
        record_sales(business_id, config_id, wa, 'Unknown', 'TBD')
